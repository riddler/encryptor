...
**scoped vault**, which is what it wants when each scope's data must be
cryptographically separable from every other scope's. A *scope* is whatever
the host keys by: here, a merchant.
...
whom must be separable and shreddable (one scope per merchant).
...
whether a vault is scoped, and a wrong guess at either changes what goes
into a message.
...
scope's KMS key and cache drainage is not what bounds it. ADR-0008 decision 4
...
| 2, scope master key | one per scope per version | 32 random bytes, wrapped by level 1, stored in your key store |
| 3, data key | one per message | generated by the engine, wrapped by level 2, discarded |

A scope master key is **32 random bytes, generated once and never derived**.
That is what makes a crypto-shred honest: destroying every copy of the wrapping
destroys the key. A key derived from the scope's id could be recomputed forever
...
shredding anything, and the shred is `ScheduleKeyDeletion` on the scope's KMS
...
**Do this before you write a single scope key.** It is the single most
...
fresh value there instead of copying **changes every scope reference in the
deployment**, orphaning every stored key name and every message header already
written, for every scope, with no way back except restoring the original root
...
The root vault's only job is to wrap and unwrap scope keys. It is an ordinary
...
every stored scope reference stays valid (ADR-0003 decision 6).
...
`Encryptor.Envelope.WrappedKey` with six fields: `scope_ref`, `version`,
...
purpose, one scope reference, one version and one namespace. A wrapping copied
...
A `:scoped` vault takes the scope as `key:` on every call, and derives the
scope reference and its context pair itself:
...
value is not secret - it is the same shape as a scope reference, which travels in
...
- **[Choosing the scope](choosing-the-scope.md)** - what the `:key` selector
  names, how to choose the boundary it draws, and the rotate/suspend/shred verbs
