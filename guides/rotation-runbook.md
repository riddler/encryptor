...
- `Encryptor.Vault.provision/2` - asks the **provider** to provision the
  ...
  scope's `CryptoKey`, which the record makes a one-time act at the scope's mint
  ...
  descriptor, keyed by `scope_ref`, never the plaintext key; persisting it is
...
re-encrypting a scope's rows would require stopping writes for the duration of
the pass.
...
> reference. Generating a new value here changes every scope reference in the
> deployment, orphaning every stored key name and every header already written,
> for every scope. It is recoverable only by restoring the original root
...
- A sample of scopes resolves: `decryption_keys/2` returns descriptors, and a
  test decrypt of one known ciphertext per sampled scope succeeds.
...
Rotates one scope's master key. **Every ciphertext for that scope is
rewritten.**
...
- **[your store]** The scope has at least one live version and its current
  version resolves.
- **[operator]** The downstream migrator has a plan covering **every** table
  and column holding that scope's ciphertext. A column missed here is a column
...
3. **[encryptor_ecto]** Run the verification pass over the same rows - **all**
...
- The scope's `decryption_keys/2` still contains **both** versions at this
...
- **[operator]** A recorded, human decision that this scope's data is to be
  ...
  automated and never a cascade from a `DELETE` on an accounts table.**
- **[operator]** Every legitimate consumer of that scope's data has been
...
1. **[your store]** Confirm the scope reference resolves and enumerate the
   wrappings that are about to be destroyed. Record the count and the version
   numbers in the change record.
2. **[your store]** Delete every wrapping for the scope. **On a
...
- **[operator]** P2 steps 2 and 3 completed and green, over all of the scope's
...
- `decryption_keys/2` for the scope no longer contains version *n*.
- The application's error rate for that scope is unchanged. **A rise in
...
It exists because P3 was otherwise the only tool for "stop serving this scope
now", and P3 cannot be undone. A suspended account, a disputed data licence, a
subject-access hold, a customer migrating out and not yet gone: all of those
want this and none of them wants a shred.
...
**Either store binds only calls through this vault.** A second client, code
that reaches the key material without this vault, or a node configured with a
different store is not denied by it. The provider locus of step 2 binds every
holder of credentials to the key material (ADR-0010 decision 9).
...
- **[operator]** Which column of the table above your vault is on is
  understood. Under the default store, the node fan-out and the post-deploy
  re-application are planned rather than remembered. Under a shared store, the
  poll interval is the delay you accept before the other nodes deny.
- **[operator]** Callers tolerate `{:key_unavailable, selector}`.
  `encryptor_ecto`'s per-owner filter shape (ece-ADR-0002 decision 11) is the
  shape that already works for this.

### Steps

1. **[encryptor]** `Encryptor.Vault.suspend(MyApp.Vault, selector)`. Under the
   default store, **on every node**. Under a shared store, once, on any node
   that shares it. An `{:error, %Encryptor.Error{reason:
   {:suspension_store_unavailable, _}}}` means the write was not confirmed:
   this node's set is unchanged, and a store that timed out may or may not
   hold it. Retry it; both verbs are idempotent.
2. **[operator]** Optionally, revoke at the provider's own backing authority -
   an IAM binding on the scope's key. This is the locus that binds more than
...
- The key store still returns the scope's rows. If it does not, this was not a
  suspension.
- Run the check against **every** node; under a shared store, allow one poll
  interval first. `suspend/2` asks no provider, so suspending a typo'd
  selector succeeds quietly; this step is what catches it.
- Under a shared store, watch `[:encryptor, :suspension, :changed]`: it fires
  on every `suspend/2` and `reinstate/2`, and on every refresh that changed the
  node's set, failed, or recovered after a failure. A node whose refreshes are
  failing is serving the last set it read.
...
A whole-scope shred is loudly distinguishable from corruption: the provider
...
Consequently, P3 step 4's deletion of the scope's ciphertext rows is
**compliance-mandatory wherever the scope's attribution is itself personal data**,
...
**A shred is wider than the columns this package encrypted.** A scope master
...
- **Never `CreateKeyRing`.** A key ring cannot be deleted, so a package that
  ...
  call, on a path reachable with a typo'd scope id.
...
grant the two use bindings (plus create, if the deployment mints scopes), and
...
- **The running process is not protected against, at all.** A process that can
  encrypt for a scope necessarily holds that scope's master key in memory,
  ...
  memory - but the unwrapped scope keys still are.
- **A scope's compromise is bounded by version, not by time.** One master key
  covers every message written while it was current. A scope compromised at
...
- The Ecto layer resolves a scope and passes it as `key:`. It does **not** put
  a scope pair in `:encryption_context` - that pair is refused from a caller -
  and its missing-scope error fires before this package is called at all.
...
a scheduler. What it will say is the argument: a scope's compromise is bounded by
