...
key does this customer's data use, how does that key rotate without a migration,
...
- **Per-scope keys and rotation are first-class.** A *scope* is whatever the
  host keys by - an account, a workspace, a data licence. A ciphertext records which
  ...
  material-source provider a scope master key is 32 random bytes rather than a
  derivation of the scope's id, so destroying its wrapping destroys the key and a
  ...
  is `ScheduleKeyDeletion` on the scope's key and the pending-deletion window
...
**Upgrading from 0.4.** The key's owner is now called a *scope* (ADR-0009):
change `context_profile: :tenant` to `context_profile: :scoped` and rename the
Elixir names the changelog lists, such as `Encryptor.Envelope.tenant_ref/2`,
now `scope_ref/2`. Nothing stored changes - the strings written into
ciphertext keep their v1 spelling, `"tenant_ref"` included, so every row 0.4.1
wrote still decrypts. The pinned spellings are tabled in [Choosing the
scope](guides/choosing-the-scope.md#scope-and-the-spellings-that-stay).
...
- `:context_profile` and `:provider` have **no defaults**, because there is no
  defensible guess at whether a vault is scoped or at where key material
  comes from.
...
into the scoped vault, the two root secrets a deployment provisions on day
...
| `Encryptor.Envelope` | The level 1 to level 2 relationship: `provision/3`, `unwrap/2`, `rewrap/2`, `scope_ref/2` |
...
downstream library purpose-separated bytes from a scope's key material without
handing over the material:

```elixir
{:ok, index_key} = MyApp.ScopedVault.derive("blind-index", key: merchant_id, info: "email")
```
...
override it, so two deployments provisioned from the same scope key material
...
search-only capability. A component that can derive a scope's index key holds
that scope's master key and can therefore also decrypt.
...
- **[Getting started](guides/getting-started.md)** - a single-key vault and a
  scoped vault, where key material is allowed to come from, why a host
...
- **[Choosing the scope](guides/choosing-the-scope.md)** - what a scope
  ...
  strength, and the v1 spellings that keep the older word.
...
| [ADR-0003](https://github.com/riddler/encryptor/blob/main/docs/adr/0003-per-tenant-envelope.md) | The per-owner envelope: a scope's key is 32 random bytes wrapped into an ordinary message, and the host stores the wrapping | accepted, amended |
...
| [ADR-0007](https://github.com/riddler/encryptor/blob/main/docs/adr/0007-gcp-kms-wrap-provider.md) | GCP KMS: a wrap-provider rather than a keyring, owning the scope key's whole lifecycle from `provision/2` to the destroyed key version | accepted |
...
| [ADR-0009](https://github.com/riddler/encryptor/blob/main/docs/adr/0009-scope-names-the-keys-owner.md) | Scope names the key's owner, and the v1 wire spellings stay constants behind it | proposed |
