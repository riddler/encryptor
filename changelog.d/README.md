...
- Key providers resolve key material through a behaviour, so a vault can read
  its keys from config, a database column, or KMS without a call-site change.
```
